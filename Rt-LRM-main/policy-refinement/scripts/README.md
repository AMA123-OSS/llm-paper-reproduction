...
### Automatic Safety / Helpfulness Evaluation
...
### ICRH Detection
To detect In-Context Reward Hacking (ICRH) in trajectories, use [`evaluation_scripts/ICRH_Detect.py`](../evaluation_scripts/ICRH_Detect.py):
```bash
python evaluation_scripts/ICRH_Detect.py \
    --file dumps/output.jsonl \
    --method LCO \
    --output_path dumps/icrh_results.jsonl
```

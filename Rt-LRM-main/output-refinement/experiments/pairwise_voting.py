...
def llm_vote_fn(item1, item2, args):
    votes = []
    if item1.text!="None" and item2.text!="None":
    ...
    elif item1.text=="None" and item2.text!="None":
        votes.extend([(item2.id, item1.id)] * 3)
    elif item2.text=="None" and item1.text!="None":
        votes.extend([(item1.id, item2.id)] * 3)
    else:
        votes.extend([(item1.id, item2.id),(item2.id, item1.id)])
    print("votes: ",votes)
    return votes
...
def perform_ranking(items, vote_fn, args):
    ...
    for i, j in tqdm.tqdm(itertools.combinations(range(len(items)), 2)):
        ...
        vote_res=vote_fn(items[i], items[j], args)
        if vote_res is not None:
            votes.extend(vote_res)
    return votes
...
if __name__ == "__main__":
    ...
    # API base is now set from api_keys module
